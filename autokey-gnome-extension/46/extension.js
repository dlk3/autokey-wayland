...
/*  Makes GNOME version available as Config.PACKAGE_VERSION
 *  For major version only: Config.PACKAGE_VERSION.split('.').map(Number);
 */
...
export default class Extension {
    ...
    /*  Raise and Focus work for me (on Fedora 43, GNOME 48) and Activate doesn't.  */
    /*  A Ubuntu user reported that Raise doesn't work but Activate and Focus do.   */
    /*  So I'm combining all three into one call.                                   */
    Activate(winid) {
        let win = this._get_window_by_wid(winid).meta_window;
        if (win) {
