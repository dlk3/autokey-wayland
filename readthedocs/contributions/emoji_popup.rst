...
::
    ...
    # Unicode characters for the emojis depicted on the buttons via
    # the keyboard.
    #
    # tkinter is not thread safe so we will run it in a separate
    # thread so that it does not interfere with AutoKey itself.
    
    # The emoji buttons to be displayed
    emoji_list = ('👌', '👍', '👎', '😀', '😢', '😮')
    ...
    #  Enable this script to write messages to the Autokey log, 
    #  ~/.local/share/autokey/autokey.log
    import logging
    from autokey.common import APP_NAME
    script_logger = logging.getLogger(APP_NAME + ".emoji.script")
        ...
        if emoji_font == '':
            ...
            for f in acceptable_fonts_list:
                if f in available_fonts_list:
                    ...
                    break
            if emoji_font == '':
               script_logger.critical('Unable to find an acceptable font on this system to display unicode emoji characters in the buttons of the popup window')
               script_logger.critical('None of these fonts are available on this system: {}'.format(acceptable_fonts_list.join(', ')))
               script_logger.critical('These are the fonts that are available on this system: {}'.format(available_fonts_list.join(', ')))
            else:
                script_logger.info('Using the "{}" font to display the emoji characters in the buttons of the popup window'.format(emoji_font))
    
        #  Name of font we'll use for non-emoji unicode characters
        text_font = 'Arial'
    ...
    #  Run the emoji_menu() function in a separate thread
